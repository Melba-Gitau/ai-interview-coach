  ...
  return (
    <div className="min-h-screen bg-gradient-page">
      ...
      <main className="mx-auto max-w-4xl px-12 py-12">
        ...
        {/* Header */}
        <div className="mb-8">
          <p className={`text-xs font-semibold px-2 py-1 rounded-full ${getTypeColor(session.type)} inline-block mb-3`}>
            {getTypeLabel(session.type)} • {session.date.toUpperCase()}
          </p>
          <h1 className="text-md font-bold text-foreground">{session.question}</h1>
        </div>
        ...
        <div className="space-y-8">
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-xl p-2 text-center">
            <p className="text-xs font-semibold text-blue-600 mb-2">OVERALL SCORE</p>
            <p className="text-2xl font-bold font-display text-blue-700">{session.score}</p>
          </div>

          {/* Your Answer */}
          <div>
            <h2 className="text-md font-semibold text-foreground mb-2">Your Answer</h2>
          ...
          <div>
            ...
            <Sparkles className="w-3.5 h-3.5 text-accent " />AI Feedback</div>
